...
image_ext = ContentData(
    data=pd.DataFrame(
        data=[["1", IMAGE_URL], ["2", AUDIO_URL], ["3", VIDEO_URL], ["3", MIMETYPE_IMAGE_URL]],
...
class TestMultimediaExtension:
    ...
    def test_extract_associate_media(self):
        ...
        """
        occ_associated_media_df = pd.DataFrame(
            data=[
                ["1", "species1", IMAGE_URL],
        ...
        dwca.extract_csv_content(
            csv_info=ContentData(data=occ_associated_media_df, keys=["occurrenceID"], type=MetaElementTypes.OCCURRENCE),
        ...
        assert sorted(list(map(attrgetter("field_name"), dwca.meta_content.meta_elements[0].fields))) == sorted(
        ...
        dwca.extract_csv_content(csv_info=associated_media_image_ext, core_ext_type=CoreOrExtType.EXTENSION)

        # Compare multimedia ext dataframe (without the coreid) against the expected image_ext dataframe
        pd.testing.assert_frame_equal(
            dwca.ext_content[0].df_content.reset_index(drop=True), image_ext.data, check_index_type=False
        )

        # Check the meta content is updated
        assert sorted(list(map(attrgetter("field_name"), dwca.meta_content.meta_elements[1].fields))) == sorted(
    ...
    def test_fill_additional_multimedia_info(self, mock_mime_types):
        ...
        dwca.extract_csv_content(
            csv_info=ContentData(
                data=pd.DataFrame(
                    data=[["1", "species1"], ["2", "species2"], ["3", "species3"]],
        ...
        expected_multimedia_df = pd.DataFrame(
            data=[
                ["1", IMAGE_URL, None, None],
    ...
    def test_fill_multimedia_info_with_format_type_partially_supplied(self, mock_mime_types):
        ...
        image_data = [
            ["1", IMAGE_URL, "image/webp", "StillImage"],
        ...
        dwca.extract_csv_content(
            csv_info=ContentData(
                data=pd.DataFrame(data=image_data, columns=["occurrenceID", "identifier", "format", "type"]),
                type=MetaElementTypes.MULTIMEDIA,
                keys=["identifier"],
            ),
        ...
        expected_multimedia_df = pd.DataFrame(
            data=expected_image_data, columns=["occurrenceID", "identifier", "format", "type"]
        )
    ...
    def test_fill_multimedia_info_type_from_format(self, mock_mime_types):
        ...
        dwca.extract_csv_content(
            csv_info=ContentData(
                data=pd.DataFrame(data=image_data, columns=["occurrenceID", "identifier", "format"]),
                type=MetaElementTypes.MULTIMEDIA,
            ),
            core_ext_type=CoreOrExtType.EXTENSION,
        )
