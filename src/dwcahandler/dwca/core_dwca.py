...
class Dwca(BaseDwca):
    ...
    def _update_column(self, df, col, other_col, stat):
        ...
        df.loc[to_update, col] = df.loc[to_update, 'dynamicProperties'].str.extract(rf'.*?"dcterms_{col}":"(.*?)"')[0]
        df.loc[to_update, 'dynamicProperties'] = df.loc[to_update, 'dynamicProperties'].str.replace(
            f'(,?)("dcterms_{col}":".*?")(?=,?)', '', regex=True).str.replace('{,', '{', regex=True)
        stat.add_update_stat(len(df[to_update]))
    ...
    def _delete_records(self, records_to_delete: CsvFileType ):
        ...
        delete_content = self._combine_contents(records_to_delete.files, records_to_delete.csv_encoding,
                                                use_chunking=False)
        valid_delete_file = all(col in delete_content.columns for col in records_to_delete.keys) or len(
            delete_content) > 0
        if not valid_delete_file:
            log.info(
                "No records removed. Delete file does not contain any records or it doesn't contain the columns: %s ",
        ...
        if core_or_ext == CoreOrExtType.CORE:
            for ext in self.ext_content:
                ...
                ext.df_content = self._delete_content(content=ext.df_content, delete_content=delete_content)
    ...
    def _read_csv(self,
        ...
        try:
            ...
            log.error(f"The expected columns: %s are not present in the {csv_file}. The file may be empty",
                      ','.join(columns))
            return pd.DataFrame()
