...
class MetaElementTypes:
    ...
    measurementorfact: ClassVar[Element] = Element("measurementorfact",
                                                   "http://rs.tdwg.org/dwc/terms/MeasurementOrFact")
    resourcerelationship: ClassVar[Element] = Element("resourcerelationship",
...
class MetaElementInfo:
    ...
    the file is core or extension, the row type, the CSV encoding and the local file name for the table."""
...
class MetaDwCA:
    ...
    def __extract_meta_info(self, ns, node_elm, core_or_ext_type):
        ...
        def extract_field_attr_value(field, attrib):
            return field.attrib.get(attrib) if field.attrib.get(attrib) else None
        ...
        if fields[0].attrib['index'] != '0':
            ...
            field_list = []
        field_list.extend(
            [Field(index=int(extract_field_attr_value(f, 'index')) if extract_field_attr_value(f, 'index') else None,
                   ...
                   vocabulary=extract_field_attr_value(f, 'vocabulary')) for i, f in enumerate(fields)])
        meta_element_attributes = MetaElementAttributes(meta_element_type=meta_element_info, fields=field_list)
    ...
    def read_meta_file(self, meta_file):
        ...
        self.meta_elements.extend([self.__extract_meta_info(ns, ne, CoreOrExtType.EXTENSION) for ne in
                                   root.findall(f'{ns}{CoreOrExtType.EXTENSION}')])
    ...
    def __get_terms(self, field):
        # Some terms from dwca contain strings like dcterms:
        col_name = self.__remove_prefix(field)
    ...
    def map_headers(self, headers: list[str], start_index: int = -1) -> list[Field]:
        ...
        for i, col in enumerate(headers):
            ...
            if i == 0:
                ...
                index += 1
            field = Field(index=index, field_name=col_name, term=self.__get_terms(col_name))
    ...
    def update_meta_element(self, meta_element_info: MetaElementInfo, fields: list[str]):
        ...
        for i, elm in enumerate(self.meta_elements):
            if elm.meta_element_type.file_name == meta_element_info.file_name:
                ...
                self.meta_elements[i] = MetaElementAttributes(meta_element_type=meta_element_info, fields=field_list)
                replace = True

        if not replace:
            field_list: list[Field] = self.map_headers(fields)
            self.meta_elements.append(MetaElementAttributes(meta_element_type=meta_element_info, fields=field_list))
    ...
    def _build_meta_xml(self, meta_elem_attrib: MetaElementAttributes):
        ...
        elem.attrib[
            'fieldsTerminatedBy'] = meta_elem_attrib.meta_element_type.csv_encoding.csv_delimiter
        ...
        elem.attrib[
            'ignoreHeaderLines'] = meta_elem_attrib.meta_element_type.ignore_header_lines
        ...
        id_field = ET.SubElement(elem,
                                 'id') if meta_elem_attrib.meta_element_type.core_or_ext_type == 'core' else ET.SubElement(
            elem, 'coreid')
        id_field.attrib['index'] = '0'

        for i, field in enumerate(meta_elem_attrib.fields):
            if field.field_name not in ('id', 'coreid'):
                field_elem = ET.SubElement(elem, "field")
                field_elem.attrib['index'] = str(field.index)
                if field.term:
                    field_elem.attrib['term'] = field.term
