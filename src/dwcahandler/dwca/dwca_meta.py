...
class MetaDwCA:
    ...
    def __extract_meta_info(self, ns, node_elm, core_or_ext_type):
        ...
        field_list.extend(
            [
                Field(
                    index=extract_field_attr_value(f, "index") if extract_field_attr_value(f, "index") else None,
        ...
        meta_element_attributes = MetaElementAttributes(
            meta_element_type=meta_element_info,
            fields=field_list,
    ...
    def remove_meta_elements(self, exts_to_remove):
        ...
        """
        self.meta_elements = [
            meta_elem for meta_elem in self.meta_elements if meta_elem.meta_element_type.file_name not in exts_to_remove
        ]
    ...
    def __get_terms(self, field_elm):
        ...
        return (
            col_name
            if len(self.terms_df[self.terms_df["term"].str.lower() == col_name.lower()]) <= 0
    ...
    def update_meta_element(self, meta_element_info: MetaElementInfo, fields: list[str], index_field: str = None):
        ...
        for i, elm in enumerate(self.meta_elements):
            if elm.meta_element_type.file_name == meta_element_info.file_name:
                ...
                if not core_id:
                    core_id = elm.core_id
                self.meta_elements[i] = MetaElementAttributes(
                    meta_element_type=meta_element_info, fields=field_list, core_id=core_id
                )
                replace = True
    ...
    def _build_meta_xml(self, meta_elem_attrib: MetaElementAttributes):
        ...
        if meta_elem_attrib.core_id:
            id_field = (
                ET.SubElement(elem, Defaults.MetaDefaultFields.ID)
